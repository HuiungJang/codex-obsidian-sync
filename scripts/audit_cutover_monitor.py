...
def validate_record(
    ...
    if record.get("launchctl_state") != "running":
        reasons.append(f"{expected_checkpoint}: launchctl print state is not running")
    ...
    if record.get("launchctl_label") != expected_label:
        reasons.append(f"{expected_checkpoint}: launchctl print header does not target expected label")
...
def summarize_record(record: dict[str, Any], path: Path) -> dict[str, Any]:
    return {
        ...
        "launchctl_label": record.get("launchctl_label"),
        "launchctl_state": record.get("launchctl_state"),
