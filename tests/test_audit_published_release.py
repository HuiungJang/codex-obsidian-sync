...
class AuditPublishedReleaseTests(unittest.TestCase):
    ...
    def test_fails_when_uploaded_evidence_summary_release_smoke_binary_does_not_match_downloaded_asset(
        self,
    ) -> None:
        with TemporaryDirectory(prefix="codex-obsidian-sync-published-release-") as temp_dir:
            root = Path(temp_dir)
            ...
            for check in summary["checks"]:
                if check["name"] == "release smoke summary:aarch64-apple-darwin":
                    check["details"]["installed_binary"] = "/tmp/other/bin/codex-obsidian-sync"
                    break
            assets["release-evidence-summary.json"] = json.dumps(summary).encode("utf-8")
            opener = FakeGitHubOpener(assets)
        ...
        self.assertIn(
            "uploaded release evidence summary check detail mismatch: "
            "release smoke summary:aarch64-apple-darwin.installed_binary",
            result["no_go_reasons"],
        )

    def test_fails_when_uploaded_evidence_summary_homebrew_prefix_does_not_match_downloaded_asset(
        self,
    ) -> None:
        with TemporaryDirectory(prefix="codex-obsidian-sync-published-release-") as temp_dir:
            root = Path(temp_dir)
            ...
            for check in summary["checks"]:
                if check["name"] == "homebrew smoke summary":
                    check["details"]["homebrew_prefix"] = "/tmp/other-codex-obsidian-sync"
                    break
            assets["release-evidence-summary.json"] = json.dumps(summary).encode("utf-8")
            opener = FakeGitHubOpener(assets)
        ...
        self.assertFalse(result["ok"])
        self.assertIn(
            "uploaded release evidence summary check detail mismatch: homebrew smoke summary.homebrew_prefix",
            result["no_go_reasons"],
        )
